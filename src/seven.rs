...
pub enum Output {
    Cmd { name: String, args: Vec<String> },
    FSO(FSO),
}
...
impl From<&str> for Output {
    fn from(value: &str) -> Self {
        ...
        return match dir_or_size {
            ...
            "dir" => Output::FSO(FSO::Dir {
                path: PathBuf::from(name),
            }),
            _ => Output::FSO(FSO::File {
                path: PathBuf::from(name),
        ...
        match output {
            ...
            Output::FSO(fso) => match fso {
                ...
                FSO::Dir { path } => {
                    let mut file_path = cwd.clone();
                    file_path.push(path);
                    fsos.push(FSO::Dir { path: file_path })
                }
            },
        }
    }
...
mod tests {
    ...
    #[test]
    fn test_fso_from_string() {
        let mut lines = SAMPLE.lines();
        let cmd1 = Output::from(lines.next().unwrap());
        assert!(matches!(cmd1, Output::Cmd { name: _, args: _ }));
        if let Output::Cmd { name, args } = cmd1 {
            assert_eq!(vec![String::from("/")], args);
        } else {
            unreachable!();
        }
        let cmd2 = Output::from(lines.next().unwrap());
        assert!(matches!(cmd2, Output::Cmd { name, args }));
        let dir = Output::from(lines.next().unwrap());
        assert!(matches!(dir, Output::FSO(FSO::Dir { path })));
        let file = Output::from(lines.next().unwrap());
        assert!(matches!(
            file,
            Output::FSO(FSO::File {
                path,
    ...
    fn test_parse() {
        ...
        assert_eq!(
            ...
            result
                .iter()
                .filter(|f| matches!(f, FSO::File { path: _, size: _ }))
                .count()
        );
    }
